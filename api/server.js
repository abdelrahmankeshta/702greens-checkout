...
// --- Price Cache for faster checkout ---
const priceCache = new Map();
const PRICE_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

async function getCachedPrice(priceId) {
    const cached = priceCache.get(priceId);
    if (cached && Date.now() - cached.timestamp < PRICE_CACHE_TTL) {
        return cached.data;
    }
    const price = await stripe.prices.retrieve(priceId);
    priceCache.set(priceId, { data: price, timestamp: Date.now() });
    return price;
}
...
app.post('/api/validate-discount', async (req, res) => {
    try {
        ...
        console.log(`[DEBUG] Min order check: orderTotal=${orderTotal}, minOrderValue=${discount.minOrderValue}`);
        if (discount.minOrderValue > 0) {
            if (!orderTotal || orderTotal < discount.minOrderValue) {
                return res.json({
                    valid: false,
                    error: `Minimum order of $${discount.minOrderValue} required for this code`
                });
            }
        }
...
// Update customer with billing details (called after Express Checkout)
app.post('/api/update-customer', async (req, res) => {
    try {
        const { customerId, email, name, phone, address } = req.body;
        ...
        console.log(`[DEBUG] Updated customer ${customerId} with billing details`);
...
// Test endpoint for Delivery Details logging
app.get('/api/test-delivery-details', async (req, res) => {
    try {
        console.log('[TEST] Testing logToDeliveryDetails...');
        await logToDeliveryDetails({
        ...
        res.json({ success: true, message: 'Check the Delivery Details sheet!' });
    } catch (err) {
...
app.get('/api/debug-sheets', async (req, res) => {
    try {
        ...
        const spreadsheetId = '1VXi95hYwsj0kUDbwgt66DnUnSv1sfUfKMax6CBWjZss';
        const metadata = await sheets.spreadsheets.get({ spreadsheetId });

        const sheetList = metadata.data.sheets.map(s => ({
        ...
        res.json({ spreadsheetId, sheets: sheetList });
    } catch (err) {
...
app.post('/api/create-subscription', async (req, res) => {
    try {
        ...
        // Fetch the price using cache for speed
        const price = await getCachedPrice(priceId);
        const isOneTime = price.type === 'one_time';

        // Check if we have add-ons (moved earlier so it's available for discount logic)
        ...
        // Fetch Add-ons if present (using cache for speed)
        if (hasAddOns) {
            const addOnPrices = await Promise.all(addOnPriceIds.map(id => getCachedPrice(id)));
        ...
        // 1) Find existing customer or create new one
        let customer;
        ...
        if (email) {
            ...
            if (existingCustomers.data.length > 0) {
                ...
                console.log(`[DEBUG] Found existing Stripe customer: ${customer.id}`);

                // Update with latest delivery info if provided
                if (delivery && delivery.firstName) {
                ...
                console.log(`[DEBUG] Created new Stripe customer: ${customer.id}`);
            }
        } else {
            // No email yet - create placeholder for Express Checkout
            customer = await stripe.customers.create({
                description: 'Guest Customer (Initialized for Express Checkout)'
            });
            console.log(`[DEBUG] Created guest customer for Express Checkout: ${customer.id}`);
        }
        ...
        console.log(`[DEBUG] Discount summary: discountDeduction=${discountDeduction}, couponId=${couponId}, isOneTime=${isOneTime}, hasAddOns=${hasAddOns}`);
        ...
        if (isOneTime && hasAddOns) {
            ...
            console.log(`[DEBUG] One-time payment: original=${originalAmount}, discount=${discountDeduction}, final=${finalAmount}`);
            ...
            */
            ...
            if (!paymentIntent && latestInvoice.amount_due === 0) {
                ...
                // This happens when:
                // 1. Discounts/coupons are applied during subscription creation
                // 2. The invoice was created but not finalized with a PI
                if (!paymentIntent && latestInvoice.amount_due > 0) {
                    console.log(`[DEBUG] Invoice ${latestInvoice.id} has no PaymentIntent. Checking invoice status: ${latestInvoice.status}`);

                    // If the invoice is still a draft, finalize it
                    ...
                    // Now retrieve the invoice again to get the PaymentIntent
                    ...
                    // FALLBACK: If still no PaymentIntent, create a standalone one
                    if (!paymentIntent) {
...
async function handlePaymentEvent(event) {
    ...
    if (eventType === 'payment_intent.succeeded') {
        ...
        // 3. Update guest customer with billing details from PaymentIntent
        // Note: Webhook payload doesn't include expanded charges, so we need to fetch them
        if (stripeCustomerId) {
            try {
                const customer = await stripe.customers.retrieve(stripeCustomerId);
                // Only update if it's a guest customer
                if (customer.description && customer.description.includes('Guest Customer')) {
                    // Fetch the PaymentIntent with charges expanded
                    const pi = await stripe.paymentIntents.retrieve(data.id, {
                        expand: ['latest_charge']
                    });

                    const billingDetails = pi.latest_charge?.billing_details || {};
                    log(`[DEBUG] Billing details from charge: ${JSON.stringify(billingDetails)}`);

                    const updateData = {};
                    if (billingDetails.email) updateData.email = billingDetails.email;
                    if (billingDetails.name) updateData.name = billingDetails.name;
                    if (billingDetails.phone) updateData.phone = billingDetails.phone;
                    if (billingDetails.address && billingDetails.address.line1) {
                        updateData.address = billingDetails.address;
                    }
                    // Clear the guest description
                    updateData.description = null;

                    if (Object.keys(updateData).length > 1) {
                        await stripe.customers.update(stripeCustomerId, updateData);
                        log(`Updated guest customer ${stripeCustomerId} with billing details: email=${billingDetails.email}, name=${billingDetails.name}`);
                        if (billingDetails.email) email = billingDetails.email;
                    } else {
                        log(`[DEBUG] No billing details to update for guest customer`);
                    }
                }
            } catch (updateErr) {
                log(`Error updating guest customer: ${updateErr.message}`);
            }
        }
    ...
    if (stripeSubscriptionId) {
        ...
        if (subscriptionId && stripeSubscriptionId) {
            ...
            // With the fixed subscription creation, the PaymentIntent should be linked to the invoice
            // When the PI is paid, the invoice/subscription should auto-activate
            try {
                const stripeSubStatus = await stripe.subscriptions.retrieve(stripeSubscriptionId);
                log(`Stripe subscription ${stripeSubscriptionId} status: ${stripeSubStatus.status}`);

                if (stripeSubStatus.status === 'active') {
                    log(`Subscription is now active in Stripe!`);
                } else if (stripeSubStatus.status === 'incomplete') {
                    // Subscription is incomplete - we need to mark the invoice as paid
                    // since we collected payment via a standalone PaymentIntent
                    log(`Subscription incomplete. Attempting to pay invoice out-of-band...`);
                    const invoiceId = stripeSubStatus.latest_invoice;

                    if (invoiceId) {
                        try {
                            const invoice = await stripe.invoices.retrieve(invoiceId);
                            log(`Invoice ${invoiceId} status: ${invoice.status}`);

                            if (invoice.status === 'open') {
                                // Mark the invoice as paid out of band (payment already collected)
                                const paidInvoice = await stripe.invoices.pay(invoiceId, {
                                    paid_out_of_band: true
                                });
                                log(`Invoice marked as paid! Status: ${paidInvoice.status}`);
                            } else if (invoice.status === 'paid') {
                                log(`Invoice already paid.`);
                            } else {
                                log(`Invoice in status: ${invoice.status}`);
                            }
                        } catch (invoiceErr) {
                            log(`Error paying invoice: ${invoiceErr.message}`);
                        }
                    }
                }
            } catch (stripeErr) {
                log(`Error checking Stripe subscription: ${stripeErr.message}`);
            }
        }
    }
    ...
    if (status === 'Succeeded' || status === 'Paid') {
        ...
        try {
            ...
            // === Log to Delivery Details (Old Database) ===
            try {
                ...
                if (stripeCustomerId) {
                    ...
                    if (stripeCustomer.address) {
                        customerZipCode = stripeCustomer.address.postal_code || '';
                    }
                }

                // Get product name from first line item
                const productName = lineItems.length > 0 ? lineItems[0].name : 'Unknown Product';

                // Determine order type for Delivery Details
                const isSubscription = orderType.includes('Subscription');

                // Get subscription info if available
                let deliveriesPerCycle = 1;
                let billingIntervalUnit = 'week';
                ...
                if (stripeSubscriptionId) {
                    try {
                        ...
                        if (stripeSub.items && stripeSub.items.data.length > 0) {
                            const plan = stripeSub.items.data[0].price;
                            if (plan.recurring) {
                                billingIntervalUnit = plan.recurring.interval;
                                billingIntervalCount = plan.recurring.interval_count;
                            }
                        }

                        // Get deliveries per cycle from Plans sheet
                        const plans = await getPlans();
                        const matchedPlan = plans.find(p =>
                            p.intervalUnit === billingIntervalUnit &&
                            p.intervalCount === billingIntervalCount
                        );
                        if (matchedPlan) {
                            deliveriesPerCycle = matchedPlan.deliveriesPerCycle;
                        }
                        ...
                        console.log(`[DEBUG] Error fetching subscription for Delivery Details: ${subErr.message}`);
                    }
                }

                // Calculate delivery date (4 days from order)
                const deliveryDate = new Date((orderDate + 4 * 24 * 60 * 60) * 1000);
                ...
                await logToDeliveryDetails({
                    ...
                    lastBillDate: new Date(orderDate * 1000),
                    nextBillDate,
                    isPaid: true,
                ...
                console.log(`[DEBUG] Logged to Delivery Details for ${stripeCustomerId}`);
            } catch (ddErr) {
                console.error('[DEBUG] Error logging to Delivery Details:', ddErr.message);
            }
...
const OLD_SPREADSHEET_ID = '1VXi95hYwsj0kUDbwgt66DnUnSv1sfUfKMax6CBWjZss'; // Old database for Delivery Details
...
// --- Delivery Details Sheet (Old Database) Integration ---
/**
 * Logs order to the "Delivery Details" sheet in the old database.
 * @param {Object} params - Order parameters
 * @param {string} params.stripeCustomerId - Stripe customer ID (cus_XXX)
 * @param {string} params.firstName - Customer first name
 * @param {string} params.lastName - Customer last name
 * @param {string} params.zipCode - Customer zip code for zone lookup
 * @param {string} params.productName - Product purchased
 * @param {string} params.orderType - 'subscription' or 'one_time'
 * @param {number} params.deliveriesPerCycle - Number of deliveries per billing cycle
 * @param {string} params.billingIntervalUnit - 'week', 'month', 'day', etc.
 * @param {number} params.billingIntervalCount - Billing interval count
 * @param {Date} params.lastBillDate - Date of last payment
 * @param {Date} params.nextBillDate - Date of next payment (null for one-time)
 * @param {boolean} params.isPaid - Whether payment was successful
 * @param {string} params.subscriptionStatus - 'active', 'incomplete', 'canceled', etc.
 * @param {Date} params.deliveryDate - Delivery date to mark
 */
async function logToDeliveryDetails(params) {
    const {
        stripeCustomerId,
        firstName,
        lastName,
    ...
    console.log(`[logToDeliveryDetails] Logging for customer ${stripeCustomerId}, product: ${productName}`);

    try {
        // === Step 1: Determine Freq value ===
        let freq = 'O'; // Default to one-time
        if (orderType === 'subscription') {
            // Map deliveriesPerCycle to Freq dropdown
            // W = 4 deliveries per cycle (weekly deliveries in a 4-week billing cycle)
            // Bi-W = 2 deliveries per cycle (bi-weekly)
            // M = 1 delivery per cycle (monthly)
            if (deliveriesPerCycle >= 4) {
                freq = 'W'; // Weekly deliveries
            } else if (deliveriesPerCycle === 2) {
                freq = 'Bi-W'; // Bi-Weekly deliveries
            } else if (deliveriesPerCycle === 1) {
                freq = 'M'; // Monthly delivery
            } else {
                freq = 'W'; // Default to weekly
            }
        }

        // === Step 2: Get Zone from Zip Code ===
        let zone = '';
        if (zipCode) {
            const zipZones = await getZipZones();
            const zoningInfo = zipZones.get(zipCode.toString().trim()) || {};
        ...
        const lastBill = formatSheetDate(lastBillDate);
        const nextBill = formatSheetDate(nextBillDate);

        // === Step 5: Build Bill Frequency text ===
        let billFrequency = '';
        if (orderType === 'one_time') {
            billFrequency = 'One-Time';
        } else {
            if (billingIntervalUnit === 'week') {
                billFrequency = billingIntervalCount === 1 ? 'Weekly' : `Every ${billingIntervalCount} weeks`;
            } else if (billingIntervalUnit === 'month') {
                billFrequency = billingIntervalCount === 1 ? 'Monthly' : `Every ${billingIntervalCount} months`;
            } else {
                billFrequency = `Every ${billingIntervalCount} ${billingIntervalUnit}`;
            }
        }

        // === Step 6: Read existing sheet data ===
        const res = await sheets.spreadsheets.values.get({
            spreadsheetId: OLD_SPREADSHEET_ID,
            range: 'Delivery Details!A:Q', // Read columns A through Q (up to Source)
        });
        const rows = res.data.values || [];

        if (rows.length === 0) {
            console.error('[logToDeliveryDetails] Sheet has no data');
            return;
        }

        // === Step 7: Find existing customer row or last customer row ===
        let existingRowIndex = -1;
        let lastCustomerRowIndex = 0; // Header is row 0

        console.log(`[logToDeliveryDetails] Sheet has ${rows.length} rows`);
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            // Check if this is the totals section (stop looking)
            if (cellA.toLowerCase().includes('total') || cellA === '-') {
                console.log(`[logToDeliveryDetails] Found totals section at row ${i + 1}`);
                break;
            }

            // This row has a customer ID (could be phone number or Stripe ID)
            lastCustomerRowIndex = i;

            // Check if this is our customer (by Stripe ID)
            if (cellA === stripeCustomerId) {
                existingRowIndex = i;
                console.log(`[logToDeliveryDetails] Found existing customer at row ${i + 1}`);
            }
        }

        console.log(`[logToDeliveryDetails] Last customer row: ${lastCustomerRowIndex + 1}, Will insert at: ${lastCustomerRowIndex + 2}`);

        // === Step 8: Prepare row data ===
        // Columns: A=ID, B=First, C=Last, D=Freq, E=Zone, F=Payment, G=Status, H=Appstle Status,
        //          I-L=skipped (Shopify Status, Zip Code, Day, Notes)
        //          M=Prod, N=Last Bill, O=Next Bill, P=Bill Frequency, Q=Source
        const rowData = [
            ...
            freq,                       // D: Freq
            zone,                       // E: Zone
            isPaid ? '' : 'UNPAID',     // F: Payment (text for now, format later)
            status,                     // G: Status
            '',                         // H: Appstle Status (empty)
            '',                         // I: (skipped)
            '',                         // J: (skipped)
            '',                         // K: (skipped)
            '',                         // L: (skipped)
            productName || '',          // M: Prod
            lastBill,                   // N: Last Bill
            nextBill,                   // O: Next Bill
            billFrequency,              // P: Bill Frequency
            'Stripe'                    // Q: Source
        ];

        // === Step 9: Update or Insert row ===
        const sheetRowNumber = existingRowIndex !== -1
            ? existingRowIndex + 1  // Update existing (1-indexed)
            : lastCustomerRowIndex + 2; // Insert after last customer (1-indexed, +1 for header, +1 for next row)

        if (existingRowIndex !== -1) {
            // Update existing customer row
            console.log(`[logToDeliveryDetails] Updating existing customer at row ${sheetRowNumber}`);
            await sheets.spreadsheets.values.update({
                spreadsheetId: OLD_SPREADSHEET_ID,
                range: `Delivery Details!A${sheetRowNumber}:Q${sheetRowNumber}`,
                valueInputOption: 'USER_ENTERED',
                requestBody: {
                    values: [rowData]
                }
            ...
            // Insert new row before totals
            console.log(`[logToDeliveryDetails] Inserting new customer at row ${sheetRowNumber}`);

            // First, insert a blank row to push totals down
            await sheets.spreadsheets.batchUpdate({
                ...
                requestBody: {
                    requests: [{
                        insertDimension: {
                            range: {
                                sheetId: 0, // Delivery Details (sheetId is unique ID, not index)
                                dimension: 'ROWS',
        ...
        // === Step 10: Set red background for unpaid ===
        if (!isPaid) {
            await sheets.spreadsheets.batchUpdate({
                spreadsheetId: OLD_SPREADSHEET_ID,
                requestBody: {
                    requests: [{
                        repeatCell: {
                            range: {
                                sheetId: 0, // Delivery Details
                                startRowIndex: sheetRowNumber - 1, // 0-indexed
                                endRowIndex: sheetRowNumber,
                                startColumnIndex: 5, // Column F (Payment)
                                endColumnIndex: 6
                            },
                            cell: {
                                userEnteredFormat: {
        ...
        // === Step 11: Handle delivery date column ===
        if (deliveryDate) {
            const deliveryDateStr = formatSheetDate(deliveryDate);

            // Get header row to find date column
            const headerRes = await sheets.spreadsheets.values.get({
                spreadsheetId: OLD_SPREADSHEET_ID,
                range: 'Delivery Details!1:1',
            });
            const headers = headerRes.data.values ? headerRes.data.values[0] : [];

            // Find column with matching date or add new one
            let dateColumnIndex = headers.findIndex(h => h === deliveryDateStr);

            if (dateColumnIndex === -1) {
                // Add new date column after last header
                dateColumnIndex = headers.length;
                console.log(`[logToDeliveryDetails] Adding new date column at index ${dateColumnIndex}: ${deliveryDateStr}`);

                // Add header
                await sheets.spreadsheets.values.update({
                    spreadsheetId: OLD_SPREADSHEET_ID,
                    range: `Delivery Details!${columnToLetter(dateColumnIndex)}1`,
                    valueInputOption: 'USER_ENTERED',
                    requestBody: {
                        values: [[deliveryDateStr]]
                    }
                });
            }

            // Mark delivery in the date column (1NM for now)
            console.log(`[logToDeliveryDetails] Marking delivery at row ${sheetRowNumber}, column ${dateColumnIndex}`);
            await sheets.spreadsheets.values.update({
                spreadsheetId: OLD_SPREADSHEET_ID,
                range: `Delivery Details!${columnToLetter(dateColumnIndex)}${sheetRowNumber}`,
                valueInputOption: 'USER_ENTERED',
                requestBody: {
                    values: [['1NM']]
                }
        ...
        console.log(`[logToDeliveryDetails] Successfully logged order for ${stripeCustomerId}`);

    } catch (error) {
        console.error('[logToDeliveryDetails] Error:', error);
    }
}

// Helper: Convert column index to letter (0 = A, 1 = B, 26 = AA, etc.)
